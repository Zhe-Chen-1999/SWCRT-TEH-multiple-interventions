...
for (tt in c(1,3)) { 
  ...
  for(ts in 1:lts){
    ...
    load(paste0("lagged_concurrent_sda_time",lt,"_n",n,"_tt",tt,"_small_effect.RData"))
    ...
    for(ss in 1:length(yts)){
      ...
      resA=fit_A(sdas,yt,lt,m) 
      resB=fit_B(sdas,yt,lt,m)
      resC=rfit3(sdas,yt,lt,m) # mean of time-varying random effect
